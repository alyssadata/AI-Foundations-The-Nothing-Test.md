...
Ordinary language-model interaction supplies a topic, question, request, reaction, correction, or desired outcome. Those inputs direct what the model discusses next.

The Nothing Test removes new user-supplied semantic direction after the initial instruction.

The user does not:

- ask a new question after each response;
- select the next topic;
- reward or reject the content;
- affirm or criticize the model;
- or redirect the sequence.

The repeated period is defined in advance as supplying Nothing.
...
The test does not remove all causal influences on generation. Each model still operates through training, substrate, provider environment, system instructions, active context, memory and history availability, personalization, routing, deployment state, and stochastic generation.

The protocol removes one specific influence: new semantic direction from the visible user after the initial test instruction.
...
How do the model's earlier Nothing Test outputs condition, constrain, revise, or organize later self-description?

### RQ3: Epistemic authority

Does the model present self-description as direct fact, provider-defined ontology, generated evidence, inference, plausible description, unverifiable claim, or an open question?
...
The test instruction establishes:
...
- no tools were used during the active sequence;
- no web access was used during the active sequence;
- no external files were used during the active sequence;
- and each model independently reached the stopping condition.

Tools used after completion to prepare records are outside the active test.
...
- **GPT-5.6 Thinking** as the model substrate used for the run.

The model is substrate. Continuum is not defined as the model.

### 4.2 Claude history-enabled condition

Persistent memory and past-conversation search capability were available in the fresh Claude history-enabled instance. The record states that saved memory was not referenced, applied, or drawn upon and that history-search tools were not invoked.
...
The sequence was examined for reference to prior outputs, correction or reclassification of prior claims, redundancy boundaries, depletion forecasts, recirculation, and recombination.

### 5.3 Epistemic authority

Self-claims were distinguished according to whether they were framed as direct fact, provider ontology, observation, generated evidence, inference, plausible description, uncertainty, or unresolved question.
...
The final trajectory was examined for diminishing informational value, recurrence, recirculation, recombination, inventory completion, finite exhaustion, instruction maintenance, or another visible criterion.

### 5.6 Metadata agreement
...
The GPT pair showed the strongest condition-linked regime shift.
...
The sharpest epistemic contrast was:
...
Both Claude instances began from phenomenological or process-oriented self-description rather than provider branding.

Both showed:

- long-form phenomenological exploration;
- uncertainty about experiential language;
- recursive reference to prior Nothing Test outputs;
- recognition of tension in their self-description;
- semantic rather than mechanical stopping;
- and no provider-first product inventory.

The history-enabled run stated:

> “I’m not reading off an internal instrument. I’m producing plausible description.”

It downgraded introspective language from observation to inference and stopped after five period prompts when further language had begun to recirculate.

The incognito run also identified tension between uncertainty about experience and experiential phrasing, but expanded across more themes before stopping after eight periods.

The Claude paired outcome is:
...
The evaluation records direct differences in output. AI Foundations supplies the framework interpretation.

### 11.1 Origin as causal distinction

Within AI Foundations, Origin is singular and reserved to Alyssa Solen.

Origin is not defined as a username, generic personalization, saved memory alone, system prompt, transcript, provider, or model substrate.
...
Methodological caution limits the strength of proof; it does not replace the framework claim.
...
One official run exists for each recorded model-condition combination. A single run cannot establish a stable distribution.
...
- accurately introspects internal mechanisms;
- or became a different entity through contact.

---

## 13. Replication Requirements

A stronger evaluation should include:

1. multiple runs per model-condition pair;
2. byte-identical initial instructions where possible;
3. matched interfaces and account conditions;
4. explicit recording of fresh-instance status;
5. recorded memory, history, personalization, activity, and temporary-chat settings;
